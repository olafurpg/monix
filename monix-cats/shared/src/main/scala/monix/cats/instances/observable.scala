...
package monix.cats.instances

import _root_.cats.Eval
import monix.cats.{Nondeterminism, Streamable}
...
trait ObservableInstances {
  ...
  val observableInstances: Streamable[Observable] with Nondeterminism[Observable] =
    new Streamable[Observable] with Nondeterminism[Observable] {
      ...
      override def startWith[A](fa: Observable[A])(elems: A*): Observable[A] =
        fa.startWith(elems:_*)
      override def startWithElem[A](fa: Observable[A])(elem: A): Observable[A] =
        Observable.concat(Observable.now(elem), fa)
      override def endWith[A](fa: Observable[A])(elems: A*): Observable[A] =
        fa.endWith(elems:_*)
      ...
      override def onErrorRetryUnlimited[A](fa: Observable[A]): Observable[A] =
        fa.onErrorRetryUnlimited
      ...
      override def zipList[A](sources: Observable[A]*): Observable[Seq[A]] =
