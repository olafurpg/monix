...
/** `BehaviorPipe` when subscribed, will emit the most recently emitted item by the source,
  ...
  *
  * When the source terminates in error, the `BehaviorPipe` will not emit any items to
  * subsequent subscribers, but instead it will pass along the error notification.
  *
  * @see [[Pipe]]
  */
final class BehaviorPipe[T] private (initialValue: T) extends Pipe[T,T] { self =>
  private[this] val stateRef = Atomic(BehaviorPipe.State(initialValue))
...
object BehaviorPipe {
  /** Builder for [[BehaviorPipe]] */
  def apply[T](initialValue: T): BehaviorPipe[T] =
    new BehaviorPipe[T](initialValue)
