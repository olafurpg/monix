...
/** Represents a [[Subject Subject]] that uses an underlying
  * [[broadcast.AsyncPipe AsyncPipe]].
  */
final class AsyncSubject[T] private
    (policy: OverflowStrategy.Synchronous, onOverflow: Long => T, s: Scheduler)
  extends PipedSubject[T,T](AsyncPipe[T](), policy, onOverflow, s)
...
object AsyncSubject {
  ...
  def apply[T](strategy: OverflowStrategy.Synchronous)
    (implicit s: Scheduler): AsyncSubject[T] =
    new AsyncSubject[T](strategy, null, s)
  ...
  def apply[T](strategy: OverflowStrategy.Evicted, onOverflow: Long => T)
    (implicit s: Scheduler): AsyncSubject[T] =
    new AsyncSubject[T](strategy, onOverflow, s)
}
